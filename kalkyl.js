...
// Funktion för att beräkna Grönt ROT (19,4%)
function calculateGronRot194(anlaggningPrisInkMoms) {
    return anlaggningPrisInkMoms * 0.194;
}

// Funktion för att beräkna inköpspris efter stöd (inkl moms) efter Grönt ROT (19,4%)
function calculateInkopPrisEfterStod194(anlaggningPrisInkMoms, gronRot194) {
    return anlaggningPrisInkMoms - gronRot194;
}

// Funktion för att beräkna el som går ut på nätet
function calculateElUtPaNatet(egenAnvandning) {
    return 100 - egenAnvandning;
}

// Funktion för att beräkna total positiv effekt av anläggning/år
function calculateTotalPositivEffektAr(egenAnvandning, beraknadEffekt, elInkop, elForsaljning, elUtPaNatet) {
    const besparingEgenAnvandEl = (egenAnvandning / 100) * beraknadEffekt * elInkop;
    const intaktFranSaldEl = (elUtPaNatet / 100) * beraknadEffekt * elForsaljning;
    return besparingEgenAnvandEl + intaktFranSaldEl;
}

// Funktion för att beräkna totalt bidrag
function calculateTotaltBidrag(gronRot50, gronRot194) {
    return gronRot50 + gronRot194;
}

// Lägg till event listener för beräkning
document.getElementById('calculatorForm').addEventListener('submit', function(event) {
    ...
    const elUtPaNatet = calculateElUtPaNatet(egenAnvandning);
    const gronRot194 = calculateGronRot194(anlaggningPrisInkMoms);
    const inkopPrisEfterStod194 = calculateInkopPrisEfterStod194(anlaggningPrisInkMoms, gronRot194);
